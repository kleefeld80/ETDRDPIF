...
function [runtime,wsoln] = enzymekinetics_2D_IMEXAdams2(dt,steps)
...
%%  matrix Assembly
e = ones(steps,1); Id = sparse(eye(nnodes));I = eye(steps);gamma = 9/16;delta = 3/2-2*gamma; beta = gamma-0.5;
...
% Semi implicit Backward Euler  
 % w_old1 = M\(w_old + dt*F(w_old));
  
% % IMEX-theta (0.5)
     w_old1 = U4\(L4\(M3*w_old + dt*F(w_old))); 
 
for i = 2:tlen-1
    w_new = U1\(L1\(M2*w_old1 + (3/2)*dt*F(w_old1)-0.5*dt*F(w_old)+beta*dt*A*w_old));
    w_old = w_old1;
    w_old1 = w_new;  
end  
runtime = toc;
