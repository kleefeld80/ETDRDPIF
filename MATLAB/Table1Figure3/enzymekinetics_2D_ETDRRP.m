...
function [runtime,w_old] = enzymekinetics_2D_ETDRRP(dt,steps)
...
for i = 2:tlen
     ...
     % ETD 1 predictor
     w_star = M1\(w_old + dt*F(w_old));
     % Full RRp
     a = M2\(9*w_old + 2*dt*F(w_old) + dt*F(w_star));
     b = M3\(-8*w_old -(3/2)*dt*F(w_old)-(dt/2)*F(w_star));
     w_old = a + b;       
end
runtime = toc;
%% Plots
