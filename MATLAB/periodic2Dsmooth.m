function [runtime] = periodic2Dsmooth(dt,steps)
...
w_old=exp(-((nodes(:,1)- 50).^2+(nodes(:,2)- 50).^2)/1000)...
     -exp(-((nodes(:,1)-100).^2+(nodes(:,2)-100).^2)/1000)...
     +exp(-((nodes(:,1)-100).^2+(nodes(:,2)- 50).^2)/1000);
...
colorbar
print -deps2c ginzburgreal2.eps
...
set(gca,'FontWeight','bold');
colorbar
print -deps2c ginzburgimag2.eps
