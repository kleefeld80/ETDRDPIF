...
function [runtime,u_soln] = Brusselator2D_IMEX_Adams2(dt,steps)
 clc;
% dt: time step (0.01)
...
alpha = 9/16;delta = 3/2-2*alpha; beta = alpha-0.5;
...
% Matrix 1
A_m = (I+alpha*4*C);
Q1 = blktridiag(A_m,-alpha*C,-alpha*C,steps);
Q1(1:2,3:4) = -2*alpha*C; Q1(ns-1:ns,ns-3:ns-2) = -2*alpha*C;
M1 = blktridiag(Q1,alpha*I2,alpha*I2,steps);
M1(1:ns,ns+1:2*ns) = alpha*I1; 
M1(nb-(ns-1):nb,nb-(2*ns-1):nb-ns) = alpha*I1;

% Matrix 2
A_m = (I-delta*4*C);
Q2 = blktridiag(A_m,delta*C,delta*C,steps);
Q2(1:2,3:4) = 2*delta*C; Q2(ns-1:ns,ns-3:ns-2) = 2*delta*C;
M2 = blktridiag(Q2,-delta*I2,-delta*I2,steps);
M2(1:ns,ns+1:2*ns) = -delta*I1; 
M2(nb-(ns-1):nb,nb-(2*ns-1):nb-ns) = -delta*I1;
...
for i = 2:tlen-1
    W_new = U1\(L1\(M2*W_old1 + (3/2)*dt*F(W_old1)-0.5*dt*F(W_old)+beta*M*W_old));
    W_old = W_old1;
    W_old1 = W_new;  
end
