function [runtime,w_old] = enzymekinetics_2D_IFETDRDP(dt,steps)
...
w_old = ones(nnodes,1);
%%  matrix Assembly
e = ones(steps,1); II = speye(nnodes);I = speye(steps);
A = (d*dt/h^2)*spdiags([-e 2*e -e], -1:1, steps, steps); % finite difference diffusion operator
A1 = kron(I,A);  % for x direction
A2 = kron(A,I);  % for y direction
M1 = sparse(II+A1);
M2 = sparse(II+(1/3)*A1); M3 = sparse(II+(1/4)*A1);
M11 = sparse(II+A2); M22 = sparse(II+(1/3)*A2); M33 = sparse(II+(1/4)*A2);
